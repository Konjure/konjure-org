...
$(document).ready(function(){var b,c,a,e;$(".material").click(function(d){if($(this).find(".materialEffect").length==0){$(this).prepend("<span class='materialEffect'></span>")
}b=$(this).find(".materialEffect");b.removeClass("animate");if(!b.height()&&!b.width()){c=Math.max($(this).outerWidth(),$(this).outerHeight());
...
})});$(document).ready(function(){$(".expander > h1, .expander > h2, .expander > h3, .expander > h4, .expander > h5, .expander > h6, .expander > p, .expander > i").click(function(){$(this).parent().toggleClass("expanded");
if($(this).parent().is("[change-icon]")){window.newIcon=$(this).parent().attr("change-icon");if($(this).parent().hasClass("expanded")){window.currentIcon=$(this).parent().find("i").attr("class").split(" ")[1];
$(this).parent().find("i").removeClass(window.currentIcon);$(this).parent().find("i").addClass(window.newIcon);
window.isChanged=true}else{$(this).parent().find("i").removeClass(window.newIcon);$(this).parent().find("i").addClass(window.currentIcon);
window.isChanged=false}}})});$(document).ready(function(){window.popped=false;window.effect="none";window.imgHeight=0;
window.imgPosition="";closePopup=function(){$(".popup").empty().remove();$(".pop").removeClass("original");
$("body").css("top",-(document.documentElement.scrollTop)+"px").removeClass("dim");$(window).scrollTop(window.scroll);
window.popped=false};addArrows=function(){$(".popup").append(b,a);window.imgHeight=($(".open").height()/2)-($(".arrow-left").height()/2);
$(".arrow-left, .arrow-right").css("top",window.imgHeight);$(".arrow-left").show();$(".arrow-right").show()
};positionAddons=function(){window.imgHeight=($(".open").height()/2)-($(".arrow-left").height()/2);var f=-Math.abs($(".caption").outerHeight());
$(".arrow-left, .arrow-right").css("top",window.imgHeight);$(".caption").css("marginTop",f)};setImgPosition=function(){if(!$(".original").parentsUntil().hasClass("wrap")){if($(".original").parent().is(":first-child")){window.imgPosition="first"
